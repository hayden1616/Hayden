...
The public catalogue now supports maintained mock product data through `public/catalog-data.js`, product details, browser-backed cart, an order-request checkout and confirmation page (no payment capture), and an attachment-enabled RFQ form at `/quote`. The homepage presents standard online ordering and custom B2B supply as equal buying paths. The front-end data adapter is intentionally isolated so it can later be replaced by a commerce API such as Medusa without changing page components. Product price, SKU, inventory, managed image upload/image URL, and publish state are maintained in SQLite through `Admin → Catalogue manager`; cart prices are refreshed from the server before checkout. Orders and RFQ metadata are stored in SQLite; checkout captures cross-border shipping details. Attachments are held in `data/uploads/`, are not publicly served, and can only be downloaded from the authenticated inquiry workspace.
